...
public class PriorityQueue<E> implements Queue<E>, Cloneable {
	...
	private void siftDownComparable(int idx, E target) {
		...
		while((child = (parent << 1)) <= size) {
			int right = child + 1;
			System.out.println("와일 시작 child: "+child+" parent: "+parent+" right: "+right);
for(int k=0; k<array.length; k++) {
				
				System.out.println("값: "+array[k]);
			}
System.out.println("-------------------------if타기전---------------------------------------------");
			Object c = array[child];
			
			//캬씨발 범인 찾앗다..재배열이 아니다 씨발아 그냥 비교만 하는거다.
			if(right <= size && ((Comparable<? super E>)c).compareTo((E)array[right]) > 0) {
				child = right;
				c = array[child];
				System.out.println("child: "+child  +" array[child]:"  + array[child]+"  right: "+right+" array[right]: "+array[right]);
			}
